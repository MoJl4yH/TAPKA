...
class Frida:
    ...
    def session(self, pid, package):
        ...
        global _FPID
        try:
            # Ensure frida-server is running before attempting to attach.
            # When frida_action="session" is used, spawn() is never called,
            # so run_frida_server() must be invoked here explicitly.
            try:
                env = Environment()
                env.run_frida_server()
            except Exception:
                logger.warning('Failed to start frida-server in session(); continuing anyway')
            try:
                device = frida.get_device(
                    get_device(),
                    settings.FRIDA_TIMEOUT)
            ...
            if session and device and _FPID:
                ...
                # In the MobSF Docker container, sys.stdin is /dev/null.
                # sys.stdin.read() returns immediately, causing the script to
                # unload before any API calls are captured.
                # Use a timed sleep so hooks remain active for data collection.
                import os as _os
                _dur = int(_os.getenv('MOBSF_FRIDA_SESSION_SEC', '45'))
                time.sleep(_dur)
                script.unload()
                session.detach()
        except frida.NotSupportedError:
