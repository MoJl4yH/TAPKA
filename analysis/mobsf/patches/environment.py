...
class Environment:
    ...
    def install_apk(self, apk_path, package, reinstall):
        ...
        # Install APK; -g grants all runtime permissions at install time to avoid
        # REVIEW_REQUIRED dialog blocking first launch on Android 6+ (API 23+).
    ...
    def system_check(self, runtime):
        ...
        try:
            ...
            if b'Read-only' in stderr:
                logger.warning(
                    'VM /system is read-only (overlayfs not yet active). '
                    'Continuing in TAPKA compatibility mode.')
                return True
        except Exception:
    ...
    def frida_setup(self):
        ...
        # frida 16+ treats SELinux Enforcing as "jailed" → NotSupportedError on spawn.
        # Disable enforcement so frida-server can inject into app processes.
        self.adb_command(['setenforce', '0'], True)

    def run_frida_server(self):
        """Start Frida Server."""
        # Re-connect to device: ADB connection may have dropped since mobsfy
        # (mobsfy calls kill-server/start-server which resets the server process;
        # minutes later the connection can be gone → frida.get_device() fails)
        logger.info('Re-connecting ADB to %s before starting Frida', self.identifier)
        try:
            subprocess.call([get_adb(), 'connect', self.identifier],
                            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            time.sleep(1)
        except Exception:
            logger.warning('ADB reconnect failed; continuing anyway')
        check = self.adb_command(['ps'], True)
        if check and b'fd_server' in check:
